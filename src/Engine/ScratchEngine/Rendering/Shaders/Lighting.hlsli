...
float Attenuate(LightSource light, float3 worldPos)
{
...
float3 DirLight(LightSource light, float3 normal, float3 worldPos, float3 camPos, float shininess, float roughness, float3 surfaceColor)
{
	...
	// Combine
    return (diff * surfaceColor + spec) * light.intensity * light.color.rgb;
}


float3 PointLight(LightSource light, float3 normal, float3 worldPos, float3 camPos, float shininess, float roughness, float3 surfaceColor)
{
	...
	// Combine
    return (diff * surfaceColor + spec) * atten * light.intensity * light.color.rgb;
}


float3 SpotLight(LightSource light, float3 normal, float3 worldPos, float3 camPos, float shininess, float roughness, float3 surfaceColor)
{
	// Calculate the spot falloff
	float3 toLight = normalize(light.position - worldPos);
    float penumbra = pow(saturate(dot(-toLight, light.direction)), light.fallOff);
...
float3 DirectionalLightPBR(LightSource light, float3 normal, float3 worldPos, float3 camPos, float roughness, float metalness, float3 albedo, float3 specularColor)
{
	// Get normalize direction to the light
	float3 toLight = normalize(-light.direction);
	float3 toCam = normalize(camPos - worldPos);
...
float3 PointLightPBR(LightSource light, float3 normal, float3 worldPos, float3 camPos, float roughness, float metalness, float3 surfaceColor, float3 specularColor)
{
	// Calc light direction
	float3 toLight = normalize(light.position - worldPos);
	float3 toCam = normalize(camPos - worldPos);
...
float3 SpotLightPBR(LightSource light, float3 normal, float3 worldPos, float3 camPos, float roughness, float metalness, float3 surfaceColor, float3 specularColor)
{
	// Calculate the spot falloff
	float3 toLight = normalize(light.position - worldPos);
	float penumbra = pow(saturate(dot(-toLight, light.direction)), light.fallOff);
